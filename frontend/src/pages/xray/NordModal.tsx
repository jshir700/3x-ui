...
export default function NordModal({
  ...
  async function fetchServers(newCountryId: number) {
    ...
    try {
      ...
      setServers(next);
      if (next.length === 0) messageApi.warning(t('pages.xray.nord.noServersFound'));
    } finally {
      setLoading(false);
    }
  }
  ...
  return (
    <>
      ...
      <Modal open={open} title={t('pages.xray.nord.title')} footer={null} onCancel={onClose}>
      {nordData == null ? (
        <Tabs
        ...
        <>
          ...
          <Button loading={loading} type="primary" danger className="mt-8" onClick={logout}>
            {t('pages.xray.nord.logout')}
          </Button>

          <Divider className="zero-margin">{t('pages.xray.nord.settings')}</Divider>

          <Form colon={false} labelCol={{ md: { span: 6 } }} wrapperCol={{ md: { span: 18 } }} className="mt-10">
            <Form.Item label={t('pages.xray.nord.country')}>
              <Select
            ...
            {cities.length > 0 && (
              <Form.Item label={t('pages.xray.nord.city')}>
                <Select
                  value={cityId}
                  showSearch
                  optionFilterProp="label"
                  onChange={setCityId}
                  options={[{ value: null, label: t('pages.xray.nord.allCities') }, ...cities.map((c) => ({ value: c.id, label: c.name }))]}
                />
              </Form.Item>
            )}

            {filteredServers.length > 0 && (
              <Form.Item label={t('pages.xray.nord.server')}>
                <Select
                  value={serverId}
                  showSearch
                  optionFilterProp="label"
          ...
          <Divider className="my-10">{t('pages.xray.nord.outboundStatus')}</Divider>
          {nordOutboundIndex >= 0 ? (
            <>
              <Tag color="green">{t('pages.xray.nord.enabled')}</Tag>
              <Button type="primary" danger loading={loading} className="ml-8" onClick={resetOutbound}>
                {t('pages.xray.nord.reset')}
              </Button>
            </>
          ) : (
            <>
              <Tag color="orange">{t('pages.xray.nord.disabled')}</Tag>
              <Button
                type="primary"
                className="ml-8"
                disabled={!serverId}
