...
export default function WarpModal({
  ...
  async function updateLicense() {
    ...
    try {
      ...
      if (msg?.success) {
        ...
        setLicenseError(msg?.msg || t('pages.xray.warp.licenseFailed'));
      }
    } finally {
      setLoading(false);
    }
  ...
  function addOutbound() {
    if (!stagedOutbound) {
      messageApi.warning(t('pages.xray.warp.fetchConfigFirst'));
      return;
    }
  ...
  return (
    <>
      ...
      <Modal open={open} title={t('pages.xray.warp.title')} footer={null} onCancel={onClose}>
      {!hasWarp ? (
        ...
        <>
          ...
          <Collapse
            ...
            items={[
              {
                key: '1',
                label: t('pages.xray.warp.licenseHeader'),
                children: (
          ...
          {hasConfig && (
            <>
              ...
              <Divider className="my-10">{t('pages.xray.warp.outboundStatus')}</Divider>
              {warpOutboundIndex >= 0 ? (
                <>
                  <Tag color="green">{t('pages.xray.warp.enabled')}</Tag>
                  <Button type="primary" danger loading={loading} className="ml-8" onClick={resetOutbound}>
                    {t('pages.xray.warp.reset')}
                  </Button>
                </>
              ) : (
                <>
                  <Tag color="orange">{t('pages.xray.warp.disabled')}</Tag>
